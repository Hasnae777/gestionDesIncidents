...
java.sql.Timestamp dateOuverture;
...
if(statut.equals("clos")){
	PreparedStatement ps2=conn.prepareStatement("select dateOuverture from ticket where idTicket="+idTicket);
	ResultSet rs=ps2.executeQuery();
	while(rs.next()){
		dateOuverture=rs.getTimestamp("dateOuverture");
	}
	
	
	PreparedStatement ps1=conn.prepareStatement("UPDATE ticket SET statut=?,dateFermeture=NOW(),dureeTache=TIMEDIFF(dateOuverture,NOW()) WHERE idTicket="+idTicket);
...
String redirectURL = "http://localhost:8080/gestionDesIncidents/ConsulterTicketDTechN.jsp?login="+nomTech;
